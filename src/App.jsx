...
const choice = {
  rock: {
    name: "Rock",
    img: "https://yt3.ggpht.com/ytc/AKedOLQfVXsfeBgPxrG4h1UL_2LXWBet3WumspjZZ7jY=s900-c-k-c0x00ffffff-no-rj",
  },
  scissors: {
    name: "Scissors",
...
function App() {
  ...
  const [compueterSelect, setCompueterSelect] = useState(null);
  const [result, setResult] = useState("");
  const [computerResult, setComputerResult] = useState("");
  const play = (userChoice) => {
    setUserSelect(choice[userChoice]);
    let computerChoice = randomChoice();
    setCompueterSelect(computerChoice);
    setResult(judgement(choice[userChoice], computerChoice));
    setComputerResult(compterJudgement(computerChoice, choice[userChoice]));
  };
  const randomChoice = () => {
  ...
  const compterJudgement = (computer, user) => {
